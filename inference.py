...
def _reasoning_score(explanation: str, min_words: int, keywords: list[str] = None) -> float:
    ...
    """
    if not explanation:
        return 0.01  # strictly > 0 per validator
    ...
    # Clamp strictly within (0.01, 0.99) — validator rejects exact 0.0 or 1.0
...
def grade_step_easy(step: int, action: MisinfoAction, sample: dict, history: list) -> MisinfoReward:
    ...
    # Strict bounds: 0.95 correct, 0.05 wrong — never exact 0.0 or 1.0
    correctness = 0.95 if correct else 0.05

    return MisinfoReward(
        total=correctness,
        correctness=correctness,
        reasoning_quality=0.01,
        source_awareness=0.01,
        efficiency=0.01,
...
def grade_step_medium(step: int, action: MisinfoAction, sample: dict, history: list) -> MisinfoReward:
    r = MisinfoReward(total=0.01, correctness=0.01, reasoning_quality=0.01,
                      source_awareness=0.01, efficiency=0.01, feedback="")
...
def grade_step_hard(step: int, action: MisinfoAction, sample: dict, history: list) -> MisinfoReward:
    ...
    if step == 0:
        ...
        r.source_awareness  = 0.1 if any(k in expl.lower() for k in source_kws) else 0.02
...
class MisinfoEnv:
    ...
    def step(self, action: MisinfoAction) -> MisinfoObservation:
        ...
        return MisinfoObservation(
            ...
            source=self._sample["source"] if self.task == "hard" else None,
            prompt=next_prompt,
            score=round(min(max(cumulative_score, 0.01), 0.99), 4),
            done=is_final,
    ...
    def state(self) -> MisinfoObservation:
        ...
        cumulative = sum(h["step_reward"] for h in self._history)
        return MisinfoObservation(
            task=self.task,
            step=self._step_idx,
            max_steps=self.config["max_steps"],
            ...
            score=round(min(max(cumulative, 0.01), 0.99), 4),
            done=self._done,
